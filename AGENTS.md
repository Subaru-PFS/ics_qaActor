...
The full suite is 91 tests and should be green. If a test module fails at *collection*, something is
...
Then confirm: `.venv/bin/python -m pytest tests -q` should report 91 passed.
...
The single `queue.Queue` is owned by the `qa` controller and handed to `Drp` in `connectionMade`, so
the controller must be attached before the model is built — that is why `attachAllControllers()`
comes first in `main.py`. The MHS model subscribed to is `drp2`, a numbered `drpActor` instance;
`opscore` strips the trailing digits, so its keys come from `actorkeys/drp.py`.
...
**`ruff check .` is not currently clean.** As of 2026-09-16 it reports 13 pre-existing errors, all
in `tests/` — mostly `N806` (camelCase locals such as `visitId`, `cmdLine`, `previousLevel`) plus
some `D401`. `N806` is explicitly selected in `[tool.ruff.lint]` but the intentional test style is
camelCase, so the config and the code disagree. Compare against the baseline rather than assuming a
non-empty report means you broke something, and **don't rename test locals to snake_case to silence
it** — either leave them or, if asked to resolve it, add `N806`/`D401` to
`per-file-ignores` for `tests/**/*.py`.
...
- **The controller thread runs for the lifetime of the actor.** `start`/`stop` on `qa` are
  ...
  `None` as a shutdown sentinel; the `run` loop must keep honouring it.
